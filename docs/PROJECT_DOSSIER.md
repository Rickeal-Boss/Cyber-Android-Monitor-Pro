...
8. **ColorOS 电池电流读数优化（参照 PlusPlusBattery 思路，2026-07-22，已修复）**：核心是**非 root 走 `BatteryManager` 属性读电流 + 用户校准倍率（battery multiplier）+ oplus_chg sysfs 作为 root/深层来源 + 零权限 SoC Δ 兜底**。
  ...
  - ⚠️ **非 root 局限**：非 root 应用 SELinux 上下文仍无法直读 `/sys/class/oplus_chg/*`（`Runtime.exec("cat")` 同上下文被拦截），故 root/深层 source 仅在已 root 设备生效；ColorOS 非 root 主修复 = 属性通路符号修复 + 校准倍率。可选后续：集成 **Shizuku** 实现非 root ADB 级 sysfs 直读，及设置 UI 暴露倍率输入。
