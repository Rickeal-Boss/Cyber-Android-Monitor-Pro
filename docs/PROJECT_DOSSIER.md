...
`refreshIntervalMs`(2000)、`cpu/gpu/memory/batteryRefreshMs`(0=全局)、`darkMode`(true)、`hapticEnabled`(true)、`hapticIntensity`(2)、`dualCellBattery`(false)、`batteryCurrentMultiplier`(1.0，钳制 [0.1,10.0]，ColorOS 等 OEM ROM 电流读数校准倍率，PlusPlusBattery 思路)、`metricCardOrder`(默认 `cpu_temp,mem_usage,battery_level,gpu_load`)、`quickCardOrder`(默认 `cpu,gpu,mem,net,gps,device,battery,sensor`)、`dashboardReorderEnabled`(true)、`appLanguage`("system")。单例用 `@Volatile` + 双重检查锁（`:20-26`），`effectiveRefreshMs(moduleMs)` 提供 0→全局回退（`:50-51`）。
...
## 4. 构建与 CI
- 单 `app` 模块；`compileSdk/target/min = 36/36/21`（compileSdk 36 受 AGP9 上限 36.1 约束；targetSdk 已于 2026-07-21 升 36，满足 2026-08-31 Play 目标 API 截止，提交 `246858f`）。
...
| 错误处理 | **存在大量空 `catch (_: Throwable) {}`**（Grep 实测 80+ 处，绝非「零空 catch」）。多数用于 OEM ROM 防御性降级静默（Camera2 常量反射、`parcel.recycle()`、隐藏 API 反射降级）；但 Battery/Gpu/DeviceDetail/System 等采集路径存在过度吞异常。**P2#10** 跟踪逐站审查——非全量替换（行为变更风险高），统一目标模式为 `try/catch(Throwable){Log.w}` 或 `runCatching{}.onFailure{ Log.w }`；AGSL 降级保留 `catch (_: Exception) { /* 静默降级 */ }`。 |
...
4. **`autoDetectDualCell()` 未实现**：`BatteryDataSource.kt:73` 仅读手动开关；`OemDataSource.kt` 已采 `chargingDualCell` 但**从未回灌** `BatteryInfo.dualCell`（数据孤岛）。
   - 🚫 **不实施（用户指令 2026-07-21 移出范围）**：双电芯自动检测按用户指令移除，不落地 `diagnosis_5_issues_fix.md` 方案 3。保留手动双电芯开关 `AppSettings.dualCellBattery`。相关数据孤岛留作后续可选优化。
...
7. **电池状态/健康硬编码中文**（`BatteryDataSource.kt:1612-1628`）+ 全仓 i18n 债（见 §8，OEM 最大来源）。
...
8. **ColorOS 电池电流读数优化（参照 PlusPlusBattery 思路，2026-07-22 落地；2026-07-22 清理 OPPO/ColorOS 死路径）**：核心是**非 root 走 `BatteryManager` 属性读电流（P0 符号修复）+ 用户校准倍率（battery multiplier）+ 零权限 SoC Δ 兜底**。原"oplus_chg sysfs 作为 root/深层来源"部分已于 2026-07-22 移除，见下方清理说明。
  - **P0 子项 — 放电电流被丢弃**：`getCurrentNowFull` 的 `BatteryManager.getIntProperty(2)`（BATTERY_PROPERTY_CURRENT_NOW）分支原 `if (current > 0)` 仅接受充电(正值)电流；放电态该属性返回负值(negative)时直接被丢弃 → 放电电流恒为 0、瓦特/内阻派生指标失真。改 `if (current != 0)` 接受带符号值，与隐藏 API 分支（线743 `!= -1 && != 0`）对齐，符合 AOSP 正=充/负=放约定（下游 `if (currentUA < 0)` 放电判定因此首次对非 root 属性通路生效）。
  - **校准倍率（battery multiplier）**：新增 `AppSettings.batteryCurrentMultiplier`（默认 1.0，钳制 [0.1, 10.0]），在电流总入口（`BatteryDataSource.kt:265-271`）对最终读数 `currentNowUA` 统一施加，自然流向瓦特/内阻/充放电判定/`normalizeBbKCurrent` 归一化。解决 ColorOS 等 OEM ROM 的 oplus_chg sysfs / property 因**单位或增益偏差**导致的读数偏大/偏小（用户在设置中校对，如读数偏大 2× 填 0.5）。倍率为 0 不修正默认行为，向后兼容。
  - **SoC Δ 零权限兜底**：当 `getCurrentNowFull` 所有路径（sysfs / property / 隐藏 API / dumpsys）均返回 0 时——正是 ColorOS "检测不到电流" 典型场景（SELinux 拦截 sysfs + `BATTERY_PROPERTY_CURRENT_NOW` 恒为 0）——由公开的电量百分比(level)与**有效满电容量**(`effectiveChargeFullMAh`，双电芯 ×2) 随时间差分反推平均电流（AccuBattery 同款 `ΔQ(mAh)=cap×Δlevel%/100 ; avg(mA)=ΔQ/Δt(h) ; ×1000→µA`）。仅在 `Δt≥60s` 差分防抖（避免整型 level 粒度与短间隔噪声放大），`±20A` sanity 上限；源标记 `SoC Δ估计`。
  - ⚠️ **非 root 局限 / 2026-07-22 清理 OPPO/ColorOS 死路径**：经 OEM 内核核查，ColorOS 13–16 对所有非 root 应用的 `oplus_chg` sysfs 读取均被 SELinux 拦截（直读与 `Runtime.exec("cat")` 同上下文均返回空/0）；唯一可用路径是 root-only 的 `/sys/class/oplus_chg/battery/bcc_parms`（逗号分隔、第 8 列为 mA），本项目不集成 root 方案。故本次移除 `getCurrentNowFull()` 的 `currentPaths` 中 22 条 OPPO/ColorOS `oplus_chg`/`vooc` 电流节点（含 ColorOS 13+/16 新增簇）与 `PathRegistry` 中死代码 `oppo` 电流条目——这些路径在 ColorOS 13–16 上恒为 0，属无效读取。**保留项**：`CURRENT_PATH_REGISTRY` 的 `oplus/vooc → ASSUME_MA` 单位提示（被 5 个单测保护，属通用转换工具，非读取方法）；容量/循环计数 `oplus_chg` 节点、温控 zone、OS 检测与 VOOC 协议识别（非电流，可能仍有效）；P0 符号修复 / 校准倍率 / SoC Δ 三项（通用逻辑，其他品牌仍受益）。可选后续：集成 **Shizuku** 实现非 root ADB 级 `bcc_parms` 直读。
...
> - ✅ 已修复：`73435ed` 落地 P0#1（charge_full 归一）、P1#5（13× waitFor 超时）、P1#6（PollingFlow 取消信号）；`246858f` store-blocking（targetSdk 36）；`b62093e` P1#7 性能模式枚举化 + P2#9 反射缓存 + P2#11 死代码 + P2#16 比较串修复（CI `29793673693` ✅）。
> - 🔶 部分修复：P1#7（BatteryScreen i18n 完成 + 性能模式枚举化；OEM 品牌专有名词与 `BatteryDataSource` 内部状态串仍待全量抽取）。
> - 🚫 不实施：P1#4（autoDetectDualCell）按用户指令 2026-07-21 移出范围。
> - P0#2（天玑规格 + 制程）、P1#3（kona 865/870）已前置提交修复。
> - ✅ 已修复：`6f67e67`+`897964d` P2#13（CI 接入单元测 + `sensor_range_*` AAPT2 报错 / 电流判定子串遮蔽 / 空区间两处实现 bug 修复，13 用例全过，CI `29795934663` ✅）；`7a3ee40` P2#12（提取 `BatteryCurrentNormalizer` 消除电流判定 God-class，CI `29796355377` ✅）。
> - 🔶 递延/不改动：P2#8（双管道，架构性留待大版本重构）、P2#10（空 catch 改逐站审查）、P2#15（阈值常量已基本命名，残留 `CpuCache.kt:636` 0.15f 不改动）。
...
- **P0（外部代码审查 🔴2 收口，2026-07-21，CI `29800457867`/`f5b4384` ✅）**：移除 `AndroidManifest.xml` 的 `ACCESS_BACKGROUND_LOCATION` 声明 + 清理 `GpsDataSource` 仅做检测、无任何功能使用的 `hasBackgroundLocation` 块（含 kdoc 第5条）。GPS 仅前台 Tab 激活时监听、无后台定位场景；该权限是 Play 后台定位策略的**发布阻断项**且从未运行时申请，移除后前台定位（FINE/COARSE）链路完整保留。提交 `f5b4384`。
  - 🔍 **辩证性复核结论（同次外部审查）**：报告所列 3 个 🔴 Blocker **仅 1 个为真**——🔴2 后台定位（已修）；🔴1 minSdk 21 与 Compose BOM 2025.06 不兼容＝**误报**（官方确认 `androidx.compose.ui:ui` 1.9.0 minSdk=21，提 23 仅见于 1.10.0-alpha01；CI 全绿印证无 manifest merger 冲突）；🔴3 内阻 irDrop 单位错误＝**误报**（算式 `|I|µA × R mΩ ÷ 1e6` 正确产出 mV，典型 1A×50mΩ→50mV 非恒 0，OCV/内阻精化逻辑正常）。9 个 🟡 中 🟡5（PendingIntent）为**非问题**（全仓仅 1 处且 `FLAG_IMMUTABLE` 正确），🟡1 计数虚高（实测 80 处非 100+）、🟡7 仅瑕疵级、🟡8 normalizer 测试已用 13 用例覆盖。真问题聚焦：🔴2（已修）+ 🟡2（PermissionHelper 废弃 API/不验证结果，已收口 `c146bc2`）+ 🟡9（移除 `-dontobfuscate` 需配真机冒烟，P2）。
- **P1（外部代码审查 🟡2 收口，2026-07-21，CI `#566`/`c146bc2` ✅）**：删除死代码 `util/PermissionHelper.kt`（全仓零外部引用，仅自身 `object` 声明；其 deprecated `startActivityForResult` 与无结果回调的 `requestOverlayPermission` 已被 `FloatingWindowScreen` 的官方 Compose 实现取代）。同步在 `FloatingWindowScreen` 新增 `DisposableEffect` + `LifecycleEventObserver`：用户拨开总开关但缺少 `SYSTEM_ALERT_WINDOW` 权限时跳转设置页并置 `pendingEnable=true`；从设置返回 `ON_RESUME` 若已授权则自动补全 `enabled=true` + `startService`，避免手动二次拨动开关（`FloatingWindowConfig.enabled` setter 同步 `_enabled` StateFlow，开关 UI 自动翻转）。提交 `c146bc2`。
- **P2（外部审查 🟡6/🟡4/🟡7 收口，2026-07-21，CI `#569`/`0a98b36`+`ea76ab0` ✅）**：三项零风险清理，**均经严格复核纠偏**——
  - 🟡6 `CpuDataSource.getTempFromSensor()`：报告称"死代码"，复核发现**并非零引用**（温度分级链 Level 4 调用点仍调用），实为**功能性死代码**——函数体永远 `return Float.NaN`（SensorManager 非回调模式无法取实时值，注释自承）。删除函数体 + Level 4 调用块，原 Level 4.5(dumpsys) 顺延为 Level 4，温度分级链更干净。
  - 🟡4 `DeviceRepository.cachedChip`：复核**确认有效**。原内联 `Class.forName("android.os.SystemProperties")…invoke("ro.board.platform")` 与紧随其后的 `SysFsReader.readProp("ro.board.platform")` 读**同一 key**、机制重复且每次重反射。改为统一走 `SysFsReader.readProp`，解析链 `socModel(API31+) → ro.board.platform → ro.hardware.chipname` 保持不变。
...
- **CI 优化（2026-07-22，CI `#576`/`60d54f9` ✅）**：debug APK 构建/上传由"每次分支 push 都跑"改为**仅 `v*-debug-pre*` tag 触发**（`on.push.tags` + 三步 `if: startsWith(github.ref,'refs/tags/')` 守卫）。分支 push 仅跑 release + 单测并跳过 debug，CI 时长缩短；打预发布 tag 时自动产出 debug+release 双包对接 prerelease 流程。
- **ColorOS 电池电流读数优化（参照 PlusPlusBattery 思路，2026-07-22，CI `29892810884` ✅）**：落地三处改动 —— (a) **P0 修复** `getCurrentNowFull` 中 `BatteryManager.getIntProperty(2)` 分支原 `if (current > 0)` 仅接受正值、丢弃放电(负)电流的 bug（改 `if (current != 0)`，与隐藏 API 分支对齐，符合 AOSP 正=充/负=放约定）；(b) 新增 `AppSettings.batteryCurrentMultiplier`(默认 1.0，钳 [0.1,10])，在电流总入口（`BatteryDataSource.kt:265-271`）对最终 `currentNowUA` 统一施加，解决 ColorOS 等 OEM ROM 的 oplus_chg / property 因单位或增益偏差导致的读数不准（用户在设置中校对，如偏大 2× 填 0.5）；(c) 新增 **SoC Δ 平均电流估计**零权限兜底——当 sysfs/property/隐藏 API/dumpsys 全部返回 0 时（ColorOS "检测不到电流" 典型场景：SELinux 拦截 sysfs + property 恒为0），由公开的电量百分比 × 有效满电容量随时间差分反推平均电流（AccuBattery 同款 ΔQ/Δt，`Δt≥60s` 防抖，`±20A` sanity），源标记 `SoC Δ估计`。下一步：设置 UI 暴露倍率输入 + （可选）Shizuku 非 root ADB 级 oplus_chg sysfs 直读。
- **P1 设置 UI 暴露电流校准倍率（2026-07-22，CI `29893718476` ✅）**：在 `BatteryScreen` 双电芯开关之下新增 `BatteryCurrentMultiplierCard`（Material3 `Slider` 0.1×–10.0× 步进 0.1× + 预设 0.5/1.0/2.0× + Reset 1.0×），直读写 `AppSettings.batteryCurrentMultiplier`；三语言包（values / zh-rCN / zh-rTW）补 `battery_current_multiplier_title/subtitle/reset` 资源。倍率字段与电流数据链路已在前一提交落地，本提交仅补 UI 入口（§10 P1#8 闭环）。
- **清理 OPPO/ColorOS 电池电流死路径（2026-07-22，CI `29903118483` ✅）**：移除 `BatteryDataSource.getCurrentNowFull()` 的 `currentPaths` 中 22 条 `oplus_chg`/`vooc` 电流读取节点（含 ColorOS 13+/16 新增簇），及 `PathRegistry` 中死代码 `oppo` 电流条目（提交 `1a20cf3`）。根因：ColorOS 13–16 非 root 应用 SELinux 拦截所有 `oplus_chg` sysfs 读取，这些路径恒为 0（唯一可用路径为 root-only 的 `bcc_parms` 第 8 列，本项目不集成）。保留：通用 `CURRENT_PATH_REGISTRY` 单位提示（5 单测保护）+ P0 符号修复 / 校准倍率 / SoC Δ + 容量/循环计数/温控/OS 检测/VOOC 协议（非电流）。
