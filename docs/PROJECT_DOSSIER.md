...
- **清理 OPPO/ColorOS 电池电流死路径（2026-07-22，CI 待验证）**：移除 `BatteryDataSource.getCurrentNowFull()` 的 `currentPaths` 中 22 条 `oplus_chg`/`vooc` 电流读取节点（含 ColorOS 13+/16 新增簇），及 `PathRegistry` 中死代码 `oppo` 电流条目。根因：ColorOS 13–16 非 root 应用 SELinux 拦截所有 `oplus_chg` sysfs 读取，这些路径恒为 0（唯一可用路径为 root-only 的 `bcc_parms` 第 8 列，本项目不集成）。保留：通用 `CURRENT_PATH_REGISTRY` 单位提示（5 单测保护）+ P0 符号修复 / 校准倍率 / SoC Δ + 容量/循环计数/温控/OS 检测/VOOC 协议（非电流）。
