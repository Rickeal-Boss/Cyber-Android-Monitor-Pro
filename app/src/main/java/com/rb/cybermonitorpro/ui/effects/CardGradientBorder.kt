 ...
 * - 靠外 1/2：天青 → 钢蓝对角渐变描边（左上 → 右下），圆角贴合卡片边框；
 ...
 * @param dynamicColor 动态描边色；非空时替代天青→蓝渐变（如电池温度语义变色），null 用默认渐变
...
fun Modifier.cardGradientBorder(
    ...
    return this
        // 层缓存：静态描边参数只随 size 失效重建一次，重放零分配；
        // 渐变 Brush 与 dynamicColor 均在 onDrawWithContent 内逐帧读取，
        // 防止主题深浅切换 / 电池温度语义色被缓存冻结
        .drawWithCache {
    ...
    // ── 默认描边光晕 Brush（onDraw 阶段现取可变 token，深浅切换后自动跟随） ──
    ...
    // ── 靠外 1/2：描边光晕（默认天青→钢蓝对角渐变；dynamicColor 非空时为纯色，逐帧求值） ──
