package com.rb.cybermonitorpro.ui.cpu
...
fun CpuScreen(
    ...
    //   替代原 groupBy { it.maxFreqKHz / 100_000 } — 整数除法会将相近频率分错组
    //   例如 2995/100=29 vs 3187/100=31, 相同 cluster 的核心被错误分为两组
    val coreGroups = cores.groupBy { it.coreCluster.ifEmpty { "cluster_${it.maxFreqKHz}" } }
    ...
    val cpuTempChart by remember(histData) { derivedStateOf { ChartUtils.normalizeChartData(histData["cpu_temp"], 100f) } }
        ...
        if (cores.isEmpty()) {
            ...
            coreGroups.values.sortedByDescending { it.first().maxFreqKHz }.forEachIndexed { groupIdx, group ->
                ...
                ClusterCard(
                    ...
                    freqData = remember(histData, clusterMaxFreq) { ChartUtils.normalizeChartData(histData["cpu_freq"], clusterMaxFreq / 1000f) }
                )
            }
        } else {
            // Per core view
            coreGroups.values.sortedByDescending { it.first().maxFreqKHz }.forEachIndexed { groupIdx, group ->
                Card(Modifier.fillMaxWidth().cardEdgeGlow(), shape = RoundedCornerShape(12.dp),
