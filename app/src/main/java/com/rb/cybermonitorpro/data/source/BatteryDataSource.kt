...
class BatteryDataSource(private val context: Context) {
    ...
    private fun getCurrentNowFull(): Pair<Long, String> {
        ...
        // ★ 原因A修复 (ColorOS 电流偏小 1000×): AOSP 标准下 getLongProperty(CURRENT_NOW)
        //   返回 µA；但 ColorOS/OPlus 等 BBK 厂商实际返回 mA 量级(小值)。单凭数值无法与
        //   AOSP 的小电流 µA 区分, 故按"厂商(BBK)+量级(<20mA)"在 normalizeBbKCurrent 校正,
        //   非 BBK 设备维持原 µA→mA 行为, 不受影响。
        try {
            val bm = appContext.getSystemService(Context.BATTERY_SERVICE) as? BatteryManager
            if (bm != null) {
                val cur = bm.getLongProperty(BatteryManager.BATTERY_PROPERTY_CURRENT_NOW) // µA 直出 (AOSP 标准；BBK 厂商的 mA 偏差于下游校正)
                if (cur != Long.MIN_VALUE) {
                    Log.d(TAG, "current_now from BatteryManager.getLongProperty(CURRENT_NOW): $cur")
