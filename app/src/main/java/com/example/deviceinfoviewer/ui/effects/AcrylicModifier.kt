 ...
 * 使用 cell-based 方式生成噪点, 每个 3×3dp cell 一个随机灰度点。
 * 性能优化: 使用 drawPoints 批量绘制 (单次 GPU 调用), 避免逐个 drawCircle。
 */
private fun Modifier.acrylicNoise(
    ...
    val noiseAlphas = mutableListOf<Float>()

    onDrawBehind {
        val cellSize = 3.5f * density // 3.5dp cell — 比 pixel-level 更自然
        if (cellSize < 1f) return@onDrawBehind

        // 首次绘制时生成噪点 (drawWithCache 保证仅一次)
        if (noisePoints.isEmpty() && size.width > 0 && size.height > 0) {
            val rng = Random(seed)
            var x = 0f
            while (x < size.width) {
                var y = 0f
                while (y < size.height) {
                    ...
                    val px = x + rng.nextFloat() * cellSize
                    val py = y + rng.nextFloat() * cellSize
                    noisePoints.add(Offset(px, py))
                    // 随机 alpha (0.3-1.0 of noiseOpacity)
                    noiseAlphas.add((0.3f + rng.nextFloat() * 0.7f) * noiseOpacity)
                    y += cellSize
                }
                x += cellSize
            }
        ...
        for (i in noisePoints.indices) {
            val alpha = noiseAlphas[i]
            if (alpha < 0.002f) continue
            drawCircle(
                color = Color.White.copy(alpha = alpha),
                radius = 0.7f,
                center = noisePoints[i]
            )
        }
    }
}
