 ...
 *
 * 性能优化 (2026-06-19):
 * - 原方案: for 循环逐点 drawCircle → 全屏覆盖层 ~23000 个噪点 = 23000 次 GPU draw call/帧
 *   文档明示 "drawCircle 等 Canvas 绘制指令构建在 CPU 端完成"，CPU 瓶颈 + draw call 风暴
 * - 新方案: drawPoints(PointMode.Points) 单次 GPU 调用批量绘制所有噪点
 *   23000 次 draw call → 1 次，CPU 开销从 O(n) 降至 O(1)
 * - 视觉折中: 放弃每点随机 alpha（微观颗粒感差异肉眼不可辨），统一 alpha = noiseOpacity
 *   噪点本质是微观纹理，统一 alpha 不影响整体质感
...
private fun Modifier.acrylicNoise(
    ...
    onDrawBehind {
        val cellSize = 3.5f * density // 3.5dp cell
