...
fun GpuScreen(
    ...
    val gpuVoltage = gpuInfo?.gpuVoltageMV?.takeIf { it > 0 }
    ...
    Column(
        ...
        MetricCard(title = "GPU load", value = load, valueColor = NeonPurpleBright,
            ...
            if (effectiveUtil != null && !effectiveUtil.isNaN()) {
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    Text("实际利用率:", fontSize = 12.sp, color = MaterialTheme.colorScheme.onSurfaceVariant)
        ...
        MetricCard(
            ...
            subtitle = maxFreq?.let { "最大: $it  ·  ${if (isThrottled) "已节流" else "正常"}" } ?: ""
        ) {
        ...
        // GPU 核心电压 (学术依据: GPULogger 论文, WiMob 2015)
        if (gpuVoltage != null) {
            MetricCard(
                title = "GPU voltage",
                value = "${gpuVoltage} mV",
