        ...
        Column(Modifier.padding(16.dp)) {
            ...
            Slider(
                ...
                onValueChange = { newVal ->
                    // ★ 吸附到最近的标准间隔值 — 替代错误的 steps 计算
                    //   原 steps=3 将 500..5000 等分为 4 段，产生 1625/2750/3875 等非标准值
                    //   新方案: 连续拖动 + 实时吸附，手指在标准值附近自动定位
                    val snapped = refreshOptions.minByOrNull { kotlin.math.abs(it - newVal.toLong()) }?.toFloat() ?: newVal
                    currentMs = snapped
                },
                ...
                steps = 0,  // ★ 连续滑块 + onValueChange 手动吸附 — 替代错误的 steps=refreshOptions.size-2
