...
private val moduleConfigs = listOf(
    ModuleIntervalConfig("CPU", Icons.Filled.PlayArrow,
        "频率、核心、温度数据刷新",
        { it.getCpuRefreshMs() }, { vm, ms -> vm.setCpuRefreshMs(ms) }),
    ModuleIntervalConfig("GPU", Icons.Filled.Settings,
        "负载、频率、温度数据刷新",
        { it.getGpuRefreshMs() }, { vm, ms -> vm.setGpuRefreshMs(ms) }),
    ModuleIntervalConfig("内存", Icons.Filled.Star,
        "内存 / ZRAM 数据刷新",
        { it.getMemoryRefreshMs() }, { vm, ms -> vm.setMemoryRefreshMs(ms) }),
    ModuleIntervalConfig("电池", Icons.Filled.Favorite,
        "容量、充放电、温度数据刷新",
        { it.getBatteryRefreshMs() }, { vm, ms -> vm.setBatteryRefreshMs(ms) }),
    ModuleIntervalConfig("网络", Icons.Filled.Share,
        "WiFi / 信号 / IP / 流量刷新",
        { it.getNetworkRefreshMs() }, { vm, ms -> vm.setNetworkRefreshMs(ms) }),
    ModuleIntervalConfig("GPS", Icons.Filled.Info,
        "卫星列表与坐标刷新",
        { it.getGpsRefreshMs() }, { vm, ms -> vm.setGpsRefreshMs(ms) }),
    ModuleIntervalConfig("传感器", Icons.Filled.Search,
        "传感器数据采集刷新",
...
private fun LanguageSettingsCard() {
    ...
    // 每次重组都重新读取当前语言，确保切换后立即反映
    val currentCode = LocaleManager.getSavedLanguage(ctx)
    val currentName = LocaleManager.SUPPORTED_LANGUAGES.find { it.code == currentCode }?.nativeName
