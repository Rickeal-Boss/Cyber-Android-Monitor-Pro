...
fun FloatingWindowScreen(onBack: () -> Unit) {
    ...
    var showCpuUsage by remember { mutableStateOf(FloatingWindowConfig.showCpuUsage) }
    ...
    Column(
        ...
        Text("悬浮窗设置", fontSize = 20.sp, fontWeight = FontWeight.Bold,
        ...
        if (enabled) {
            ...
            CheckItem("CPU 频率 (最高核心)", showCpuFreq) { showCpuFreq = it; FloatingWindowConfig.showCpuFreq = it }
            CheckItem("运行内存", showRam) { showRam = it; FloatingWindowConfig.showRam = it }
            CheckItem("电池温度", showBatteryTemp) { showBatteryTemp = it; FloatingWindowConfig.showBatteryTemp = it }
            CheckItem("电池电流", showBatteryCur) { showBatteryCur = it; FloatingWindowConfig.showBatteryCurrent = it }
            CheckItem("电池功率 (V×I)", showBatteryPow) { showBatteryPow = it; FloatingWindowConfig.showBatteryPower = it }

            Text("系统", fontSize = 16.sp, fontWeight = FontWeight.SemiBold,
                color = MaterialTheme.colorScheme.onSurface,
                modifier = Modifier.padding(top = 8.dp))
            CheckItem("实时 FPS", showFps) { showFps = it; FloatingWindowConfig.showFps = it }
        }

        Spacer(Modifier.height(16.dp))
        Text("提示: Android 14+ 需手动授予 \"显示在其他应用上层\" 权限",
            fontSize = 12.sp,
