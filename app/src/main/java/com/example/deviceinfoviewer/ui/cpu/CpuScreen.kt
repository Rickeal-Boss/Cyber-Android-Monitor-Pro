...
fun CpuScreen(
    ...
    val availableGovernors = cpuInfo?.availableGovernors?.takeIf { it.isNotEmpty() }
    val currentGovernor = cores.firstOrNull()?.governor?.takeIf { it.isNotEmpty() }
    ...
    Column(
        ...
        // CPU 调速器 (学术依据: BLAST/GPULogger 论文 — governor 对功耗和性能影响巨大)
        if (currentGovernor != null) {
            MetricCard(
                title = "CPU Governor",
                value = currentGovernor,
                valueColor = NeonPurpleBright,
                subtitle = availableGovernors ?: ""
            )
        }
