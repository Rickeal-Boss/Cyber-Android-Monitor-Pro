...
fun CpuScreen(
    ...
    val cpuTempChart by remember(histData) { derivedStateOf { normalizeChartData(histData["cpu_temp"], 100f) } }
    val cpuFreqChart by remember(histData) { derivedStateOf { normalizeChartData(histData["cpu_freq"], 3500f) } }
    val deepSleepChart by remember(histData) { derivedStateOf { normalizeChartData(histData["cpu_deep_sleep"], 100f) } }
        ...
        if (cores.isEmpty()) {
            ...
            coreGroups.values.sortedByDescending { it.first().maxFreqKHz }.forEach { group ->
                ...
                ClusterCard(
                    ...
                    freqData = remember(histData, clusterMaxFreq) { normalizeChartData(histData["cpu_freq"], clusterMaxFreq / 1000f) }
                )
            }
        } else {
            // Per core view
