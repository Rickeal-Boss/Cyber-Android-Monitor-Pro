    ...
    Column(
        ...
        if (deepSleepPct != null && !deepSleepPct.isNaN() && cStates.isNotEmpty()) {
            ...
            cStates.forEach { state ->
                ...
                MetricCard(
                    title = "${state.name} (C${state.level}·${cStateDesc(state.name, state.level)})",
                    value = "${pct.toInt()}%",
                    valueColor = color,
    ...
    Card(Modifier.fillMaxWidth().padding(horizontal = 2.dp), shape = RoundedCornerShape(8.dp),
        ...
        Column(Modifier.padding(12.dp)) {
            Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                ...
                if (!core.online) Text("OFF", fontSize = 11.sp, color = NeonMagenta)
            ...
            Text("max: ${core.maxFreqKHz / 1000} MHz · ${core.governor ?: "-"}", fontSize = 11.sp, color = MaterialTheme.colorScheme.onSurfaceVariant)
        }
    }
}

// ── C-State 中文注解 ──
private fun cStateDesc(name: String, level: Int): String = when {
    name.contains("WFI", ignoreCase = true) || level == 1 -> "等待中断(轻量睡眠)"
    name.contains("C1", ignoreCase = true) -> "时钟门控·唤醒<1µs"
    name.contains("C2", ignoreCase = true) -> "停用核心时钟·唤醒<2µs"
    name.contains("C3", ignoreCase = true) -> "关闭PLL·缓存保持·唤醒<50µs"
    name.contains("retention", ignoreCase = true) || name.contains("Ret", ignoreCase = true) -> "核心下电·状态保持(Retention)"
    name.contains("power collapse", ignoreCase = true) || name.contains("Collapse", ignoreCase = true) -> "完全断电·上下文丢失"
    name.contains("Deep", ignoreCase = true) || name.contains("deep", ignoreCase = true) -> "深度睡眠·唤醒<1ms"
    name.contains("cluster", ignoreCase = true) -> "集群级睡眠·共享L2保持"
    name.contains("idle", ignoreCase = true) -> "空闲状态"
    else -> when (level) {
        0 -> "WFI 等待中断"
        1 -> "C1 时钟门控"
        2 -> "C2 停用时钟"
