...
fun OemScreen(viewModel: OemViewModel = koinViewModel()) {
    ...
    Column(
            ...
            if (hasHyperFeatures) {
                SectionCard("HyperOS 特性") {
                    ...
                    RowItemWithColor("性能评级", o.hyperOsPerformanceGrade.ifEmpty { "-" },
                        if (o.hyperOsPerformanceGrade == "性能模式") SuccessNeon else MaterialTheme.colorScheme.onSurface)
                }
            }
        ...
        SectionCard("性能模式") {
            ...
            RowItemWithColor("高性能模式", if (o?.highPerformanceMode == true) "已开启 ✓" else "未开启", perfColor)
        }
