    ...
    Column(
        ...
        SectionCard(if (hasRealGpu) "GPU 图形 · ${detail!!.glRenderer}" else "GPU 图形") {
            ...
            if (keyExts != null && keyExts.isNotEmpty()) {
                RowItem("GL 扩展", keyExts.take(6).joinToString(" · ") +
                    if (keyExts.size > 6) " +${keyExts.size - 6}项" else "")
            } else {
        ...
        SectionCard("多媒体解码器") {
            ...
            if (v.isNotBlank()) RowItem("视频解码", v + if (vCount > 8) " (+${vCount - 8})" else "")
            val a = detail?.audioCodecs?.take(8)?.joinToString(", ").orEmpty()
            if (a.isNotBlank()) RowItem("音频解码", a + if (aCount > 8) " (+${aCount - 8})" else "")
            if (hwCount > 0) {
                val hw = detail?.hwAcceleratedCodecs?.take(8)?.joinToString(", ").orEmpty()
                RowItem("HW 加速", hw + if (hwCount > 8) " (+${hwCount - 8})" else "")
            }
        }

        // ═══════ 13. 热管理 (新增) ═══════
        if (detail?.thermalZoneCount?.compareTo(0) == 1 || detail?.coolingDeviceCount?.compareTo(0) == 1) {
            SectionCard("热管理") {
                RowItem("热区传感器", "${detail?.thermalZoneCount ?: 0} 个")
                ...
                // 冷却设备 (学术依据: DATE 2019 热管理论文)
                if (detail?.coolingDeviceCount?.compareTo(0) == 1) {
                    RowItem("冷却设备", "${detail!!.coolingDeviceCount} 个")
                    val cStates = detail!!.coolingDeviceStates
                    if (cStates.isNotEmpty()) {
                        RowItem("状态", cStates.take(5).joinToString("  |  ") +
                            if (cStates.size > 5) " …" else "")
                    }
                }
            }
        }
        ...
        // ═══════ 13.5. 存储 IO (新增) ═══════
        if (detail?.ioScheduler?.isNotEmpty() == true) {
            SectionCard("存储 IO 调度器") {
                RowItem("当前调度器", detail!!.ioScheduler)
            }
        }
