    ...
    Column(
        ...
        if (vkVersion != null || vkLevel != null || vkExts != null || vkDevCount != null) {
            SectionCard("Vulkan") {
                if (vkVersion != null) {
                    ...
                    RowItem("API 版本", detailed)
                }
                if (vkLevel != null) RowItem("硬件级别", vkLevel)
                if (vkDevCount != null) RowItem("物理设备数", "$vkDevCount 个")
                if (vkExts != null) {
                    val keyExts = listOf(
                    ...
                    val summary = if (found.isNotEmpty()) {
                        "已启用 ${found.size}/$total 项关键扩展 · 共 $total 项"
                    } else {
                        "共 $total 项扩展"
                    }
                    RowItem("扩展", summary)
        ...
        if (hasAndroidId || hasSerial || hasHwSerial || hasFingerprint) {
            SectionCard("设备标识符") {
                ...
                if (hasSerial) {
                    RowItem("序列号", detail!!.serialNumber,
                        valueColor = NeonCyan)
                }
                if (hasHwSerial) {
        ...
        if (oem != null && oem!!.oem != "AOSP") {
            ...
            SectionCard("性能模式") {
                ...
                RowItem("当前调度", oem!!.powerModeCurrent.ifEmpty { "均衡模式" })
            }
