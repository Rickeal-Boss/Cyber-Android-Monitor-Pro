...
fun NetworkScreen(viewModel: NetworkViewModel = koinViewModel()) {
    ...
    val wifiSpeedChart by remember(historyData) {
        derivedStateOf { normalizeChartData(historyData["wifi_speed"], 1000f) }
    }
    val signalChart by remember(historyData) {
        derivedStateOf { normalizeChartDataAbs(historyData["signal_strength"]) }
    }
...
private fun normalizeChartData(points: List<HistoryDataPoint>?, maxValue: Float): List<Float> {
    ...
    // ★ 图表点上限 80 (2026-06-21): 防止数据量膨胀导致绘制压力
    val recent = points.takeLast(80)
    return if (maxValue > 0) {
        val safeMax = if (maxValue.isInfinite() || maxValue.isNaN()) 1f else maxValue
        recent.map { (it.value / safeMax).coerceIn(0f, 1f) }
    } else recent.map { it.value.coerceIn(0f, 1f) }
}
