...
fun NetworkScreen(viewModel: NetworkViewModel = koinViewModel()) {
    ...
    Column(
        ...
        // ── NR/LTE 独立信号强度 dBm ──
        if (nrDbm != null) {
            val nrLevel = signalLevelText(nrDbm, -90, -105, -115)
            MetricCard(
                title = "NR 5G 信号强度",
                value = "$nrDbm dBm  ·  $nrLevel",
                valueColor = signalLevelColor(nrDbm, -80, -100)
            )
        }
        if (lteDbm != null) {
            val lteLevel = signalLevelText(lteDbm, -90, -105, -115)
            MetricCard(
                title = "LTE 4G 信号强度",
                value = "$lteDbm dBm  ·  $lteLevel",
                valueColor = signalLevelColor(lteDbm, -80, -100)
            )
        }
        // 通用信号强度卡片（RSRP 兜底）
        if (signalStrength != null && nrDbm == null && lteDbm == null) {
            val pct = kotlin.math.min(100, (signalStrength + 120) * 100 / 60).coerceIn(0, 100)
            MetricCard(title = "信号强度", value = "$signalStrength dBm · $pct%",
    ...
    Card(
        ...
        Column(Modifier.padding(16.dp)) {
            ...
            if (info.dlBandwidth.isNotEmpty()) {
                CellRow("DL 带宽", info.dlBandwidth)
            }
            // UL Configured
            if (info.ulConfigured.isNotEmpty()) {
                CellRow("UL 状态", info.ulConfigured)
            }
            // RSRP
            if (info.rsrp > Int.MIN_VALUE) {
                CellRow("RSRP", "${info.rsrp} dBm",
                    signalLevelColor(info.rsrp, -80, -100))
            }
            // RSRQ
