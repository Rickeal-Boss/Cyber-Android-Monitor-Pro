...
data class BatteryInfo(
    ...
    /**
     * 电池健康度 (State of Health)
     * 定义: 当前满电容量 / 设计满电容量 × 100%
     * 学术依据: ICCPS 2017 "Battery State-of-Health Estimation for Mobile Devices"
     *   - Android 只报告 "good/dead"，实测可达 45% 容量衰减仍显示"good"
     *   - SOH 定义为 full_charge_capacity / rated_capacity
     */
    val sohPercent: Int?
        get() = if (chargeFullMAh > 0 && chargeFullDesignMAh > 0)
            (chargeFullMAh * 100 / chargeFullDesignMAh).toInt().coerceIn(0, 100)
        else null
}
