...
import com.example.deviceinfoviewer.data.model.CpuCState
...
class CpuDataSource(private val context: Context) {
    ...
    fun getCpuInfo(): CpuInfo {
        ...
        val updatedSlots = if (refreshGovernor) cachedCoreTopology.toMutableList() else mutableListOf()
     ...
     */
    fun collectCpuIdleStats(): Quadruple<Float, List<CpuCState>, Float, String> {
        try {
            // 聚合所有核心的各 C-State
            val allStates = mutableMapOf<String, MutableList<CpuCState>>()
            var coresFound = 0
