...
enum class SensorTypeMeta(
    ...
    companion object {
         ...
         * 距离传感器多档状态描述 (cm)
         */
        fun describeProximityState(distance: Float, maxRange: Float, context: Context): String = when {
            distance <= 0f -> context.getString(R.string.proximity_state_contact)
            distance <= 0.5f -> context.getString(R.string.proximity_state_near)
            distance <= 2f -> context.getString(R.string.proximity_state_close)
            distance <= maxRange * 0.25f -> context.getString(R.string.proximity_state_fair, "%.1f".format(maxRange * 0.25f))
            distance <= maxRange * 0.5f -> context.getString(R.string.proximity_state_mid, "%.1f".format(maxRange * 0.5f))
            distance <= maxRange * 0.75f -> context.getString(R.string.proximity_state_far, "%.1f".format(maxRange * 0.75f))
