        ...
        for (sensor in sensors) {
            result.add(
                SensorItemInfo(
                    ...
                    typeName = SensorTypeMeta.getDisplayName(sensor.type, context)
                )
            )
        }
    ...
    fun startListening(sensorType: Int, callback: (SensorLiveData) -> Unit) {
        ...
        try {
            val samplingPeriod = when (sensorType) {
                Sensor.TYPE_LIGHT -> 50_000        // 光线传感器: 50ms 精细采样 (全量程识别)
                Sensor.TYPE_PROXIMITY -> 30_000    // 距离传感器: 30ms 高频连续采样 (多距离追踪)
