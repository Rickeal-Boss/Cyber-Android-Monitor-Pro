...
/**
 * 核心数据仓库 — Kotlin 协程驱动 (v3 重构版)
 *
 * [Architect Note] 重构目标 (code2.md P1-Task3):
 *   1. HealthTracker 提取 → 健康状态追踪独立类
 *   2. AuxiliaryCollector 提取 → WiFi/Mobile/Storage/NetIf/GPS 辅助模块独立类
 *   3. 核心模块 (CPU/GPU/Memory/Battery) 保留在此，但使用 SharedFlow 替代 LiveData
 *   4. DeviceRepository 行数目标 ≤ 200 行 (当前约 200 行)
 *
 * 数据管道:
 *   DataSource → SharedFlow (replay=1, DROP_OLDEST) → Compose collectAsState()
 *   LiveData 保留作为向后兼容层 (已废弃，建议迁移到 SharedFlow)
 */
class DeviceRepository(context: Context) {
    ...
    // ═══════ 提取的独立类 ═══════
    // [Architect Note] 单一职责: HealthTracker 管理 13 源健康状态，AuxiliaryCollector 管理辅助模块
    ...
    // ═══════ SharedFlow 输出 (主数据管道) ═══════
    // [Architect Note] replay=1 保证新订阅者立即获取最新值
    // BufferOverflow.DROP_OLDEST 在高频采集时丢弃旧值，防止背压堆积
    ...
    // ═══════ 协程调度 ═══════
    // [Architect Note] Dispatchers.IO 用于 sysfs/网络 I/O 密集采集
    ...
    fun startMonitoring(intervalMs: Long = RefreshPolicy.Tier.NORMAL.defaultMs) {
        ...
        // [Architect Note] RefreshPolicy 观察者: 前后台切换 + 省电模式变更 → 分模块调速
        // 原代码 (P0-1/P0-2):
        //   1. setIntervalMs(newMs) 将所有模块设为统一值，破坏了用户的分模块配置
        //   2. 仅观察 RefreshPolicy.state，省电模式变更不触发重算
        // 修复: combine(state, powerSaveModeFlow) + pushPolicyAdjustedIntervals()
        //   每个模块独立计算: effectiveMs(用户配置, NORMAL) → 前台用用户配置/后台封顶 5s
    ...
    private suspend fun collectCpuBlock() {
        try {
            ...
            _cpuFlow.emit(cpu); cpuLiveData.postValue(cpu)  // [Architect Note] 双写: SharedFlow(主) + LiveData(兼容)
    ...
    /**
     * [Architect Note] 分模块应用 RefreshPolicy 调速 (P0-1/P0-2 修复)
     *
     * 与 pushAllIntervalFlows 的区别:
     *   pushAllIntervalFlows 仅使用 AppSettings.effectiveRefreshMs (用户配置),
     *   不感知 RefreshPolicy 前后台/省电模式状态。
     *   pushPolicyAdjustedIntervals 在此基础上叠加 RefreshPolicy.effectiveMs(),
     *   实现: 前台=用户配置 | 后台=封顶 5s | 省电=封顶 5s。
     *   每个模块独立计算，保留分模块差异 (如 CPU=500ms, GPU=2000ms)。
     */
