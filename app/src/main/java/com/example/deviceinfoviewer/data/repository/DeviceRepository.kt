...
class DeviceRepository(context: Context) {
    ...
    private suspend fun collectData() = withContext(Dispatchers.Default) {
        ...
        runCatching {
            ...
            // 新增: 容量衰减追踪 (charge_full / charge_full_design)
        ...
        historyData.postValue(
            mapOf(
                ...
                "battery_soh" to historyCache.getSeries("battery_soh"),
                "battery_charge_full" to historyCache.getSeries("battery_charge_full"),
                "battery_resistance" to historyCache.getSeries("battery_resistance"),
    ...
    fun setCpuRefreshMs(ms: Long) = AppSettings.getInstance(context).apply { cpuRefreshMs = ms.toInt() }
    fun getCpuRefreshMs(): Long = AppSettings.getInstance(context).cpuRefreshMs.toLong()
    fun setGpuRefreshMs(ms: Long) = AppSettings.getInstance(context).apply { gpuRefreshMs = ms.toInt() }
    fun getGpuRefreshMs(): Long = AppSettings.getInstance(context).gpuRefreshMs.toLong()
    fun setMemoryRefreshMs(ms: Long) = AppSettings.getInstance(context).apply { memoryRefreshMs = ms.toInt() }
    fun getMemoryRefreshMs(): Long = AppSettings.getInstance(context).memoryRefreshMs.toLong()
    fun setBatteryRefreshMs(ms: Long) = AppSettings.getInstance(context).apply { batteryRefreshMs = ms.toInt() }
    fun getBatteryRefreshMs(): Long = AppSettings.getInstance(context).batteryRefreshMs.toLong()
    fun setNetworkRefreshMs(ms: Long) = AppSettings.getInstance(context).apply { networkRefreshMs = ms.toInt() }
    fun getNetworkRefreshMs(): Long = AppSettings.getInstance(context).networkRefreshMs.toLong()
    fun setGpsRefreshMs(ms: Long) = AppSettings.getInstance(context).apply { gpsRefreshMs = ms.toInt() }
    fun getGpsRefreshMs(): Long = AppSettings.getInstance(context).gpsRefreshMs.toLong()
    fun setSensorsRefreshMs(ms: Long) = AppSettings.getInstance(context).apply { sensorsRefreshMs = ms.toInt() }
    fun getSensorsRefreshMs(): Long = AppSettings.getInstance(context).sensorsRefreshMs.toLong()
}
