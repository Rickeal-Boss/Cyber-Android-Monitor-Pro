...
class DeviceRepository(context: Context) {
    ...
    // ★ 当前全局刷新间隔（仅用于 getIntervalMs 查询，实际驱动由 StateFlow 完成）
    @Volatile
    private var intervalMs: Long = RefreshPolicy.Tier.NORMAL.defaultMs
    ...
    private suspend fun collectBatteryBlock() {
        try {
            ...
            // ★ 系统省电模式 → RefreshPolicy 联动: 自动降频至 5s 封顶
    ...
    fun setIntervalMs(ms: Long) {
        ...
        this.intervalMs = ms
        // ★ 直接写入各模块 StateFlow (绕过 AppSettings 分模块配置)
        //   后台降频/前台恢复 → 所有模块统一为新间隔
        //   分模块 AppSettings 在 pushAllIntervalFlows() (启动时) 中独立使用
