...
class OemDataSource(private val context: Context? = null) {
        ...
        try {
            ...
            for (id in manager.cameraIdList) {
                try {
                    ...
                    if (oisModes != null) {
                        if (oisModes.contains(CameraCharacteristics.LENS_OPTICAL_STABILIZATION_MODE_ON)) {
    ...
    private fun detectPerformanceModes(info: OemInfo, oem: String) {
        when (oem) {
            ...
            OEM_SAMSUNG -> {
                info.gameModeSupported = prop("persist.sys.gamemode", "0") == "1"
                    || prop("sys.gamedvfs", "0") == "1"
