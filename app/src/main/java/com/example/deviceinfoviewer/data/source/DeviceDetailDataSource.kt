...
class DeviceDetailDataSource(private val context: Context) {
    ...
    private fun collectAudio(info: DeviceDetailInfo) {
        try {
            ...
            // 声道数: 使用 AudioDeviceInfo.getChannelCounts() 而非 PROPERTY_OUTPUT_FRAMES_PER_BUFFER
            // 后者返回的是音频缓冲区帧数，不是声道数
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                try {
                    ...
                    if (speaker != null) {
                        val channelCounts = speaker.channelCounts // 返回支持的声道数数组
                        val maxChannels = channelCounts.maxOrNull() ?: 2
                        info.audioOutputChannels = when {
                            maxChannels >= 8 -> "7.1 环绕 ($maxChannels 声道)"
                            maxChannels >= 6 -> "5.1 环绕 ($maxChannels 声道)"
                            maxChannels >= 4 -> "4 声道"
                            maxChannels >= 2 -> "立体声 (双声道)"
                            else -> "单声道"
                        }
                    } else {
                        // 没有内置扬声器信息，尝试所有输出设备
                    ...
                    info.stereoSpeakers = devices.count {
                        it.type == AudioDeviceInfo.TYPE_BUILTIN_SPEAKER
                    } >= 2
                } catch (_: Throwable) {
                    // 回退: 检查属性
                    val channels = am.getProperty(AudioManager.PROPERTY_OUTPUT_FRAMES_PER_BUFFER)
                    if (!channels.isNullOrEmpty() && channels != "0") {
                        val bufFrames = channels.toIntOrNull() ?: 0
                        info.audioOutputChannels = when {
                            bufFrames >= 768 -> "立体声"  // 大缓冲区暗示多通道
                            bufFrames >= 256 -> "立体声"
                            else -> "单声道"
                        }
                    }
