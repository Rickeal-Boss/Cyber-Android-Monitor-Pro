...
class DeviceDetailDataSource(private val context: Context) {
            ...
            dm?.getDisplay(android.view.Display.DEFAULT_DISPLAY)?.let { display ->
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                    try {
                        ...
                        for (type in hdr.supportedHdrTypes) {
                            types.add(when (type) {
    ...
    private fun collectWifiStandard(info: DeviceDetailInfo) {
        try {
            ...
            // 策略1: WifiInfo.getWifiStandard() (API 33+)
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                try {
                    val wm = context.getSystemService(Context.WIFI_SERVICE) as? WifiManager
                    val connectionInfo = wm?.connectionInfo
                    if (connectionInfo != null) {
                        val standard = connectionInfo.wifiStandard
        ...
        try {
            ...
            for (id in camIds) {
                try {
                    ...
                    sensor.oisSupported = oisModes?.any { it == CameraCharacteristics.LENS_OPTICAL_STABILIZATION_MODE_ON } == true
