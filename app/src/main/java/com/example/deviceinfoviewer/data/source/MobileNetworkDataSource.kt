...
class MobileNetworkDataSource(private val context: Context) {
    ...
    private fun parseNr(nr: CellInfoNr, info: MobileNetworkInfo): Boolean {
        try {
            ...
            // 分制式信号强度 (NR)
            info.nrRsrp = info.rsrp
            if (info.rsrp != Int.MIN_VALUE) {
                info.nrSignalDbm = info.rsrp  // RSRP 本身就是 dBm
            }
            // 如果 RSRP 不可用，尝试用 SS-RSRP 回退
            if (info.nrSignalDbm == Int.MIN_VALUE) {
                info.nrSignalDbm = try { signal.javaClass.getMethod("getSsRsrp").invoke(signal) as? Int ?: Int.MIN_VALUE } catch (_: Throwable) { Int.MIN_VALUE }
            }
