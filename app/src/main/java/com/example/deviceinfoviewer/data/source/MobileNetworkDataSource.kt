...
class MobileNetworkDataSource(private val context: Context) {
    ...
    private fun parseNr(nr: CellInfoNr, info: MobileNetworkInfo): Boolean {
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                try {
                    // CellIdentityNr 无直接带宽字段，通过 nrarfcn 推算
                    val ndlBw = identity.bandwidth
                    if (ndlBw > 0) {
                        info.dlBandwidth = "${ndlBw} MHz"
                    }
                } catch (_: Throwable) {}
            }
            // 带宽回退：通过 ARFCN 推算常见 NR 带宽
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                try {
                    val rssiField = CellSignalStrengthNr::class.java.getDeclaredField("rssi")
                    rssiField.isAccessible = true
                    info.rssi = rssiField.getInt(signal)
                } catch (_: Throwable) {}
            }
