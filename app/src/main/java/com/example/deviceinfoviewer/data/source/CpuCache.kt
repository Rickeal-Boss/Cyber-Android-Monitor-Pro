...
object CpuCache {
    ...
    fun injectCpuInfo(chip: KnownChip, info: CpuInfo) {
        ...
        if (info.cores.isEmpty()) {
            ...
            // [Architect Note] 基于频率匹配 coreType，替代按索引顺序赋值
            // 原代码按 chip.clusters 顺序赋值 coreName，但 sysfs 返回的核心顺序
            // 与芯片数据库中的 cluster 顺序不一致（如大核 cluster 在 sysfs 排第 4 位
            // 但在 chip.clusters 中排第 1 位），导致 coreType 覆盖错位。
            // 修复: 对每个核心，用 maxFreqKHz 匹配最近的 cluster (1% 容差)。
            for (core in info.cores) {
                val bestCluster = chip.clusters.minByOrNull { cluster ->
                    ...
                    kotlin.math.abs(core.maxFreqKHz - clusterFreqKHz)
                }
                if (bestCluster != null) {
                    val diffPercent = if (core.maxFreqKHz > 0) {
                        kotlin.math.abs(core.maxFreqKHz - (bestCluster.maxFreqGHz * 1_000_000).toLong()).toFloat() / core.maxFreqKHz
                    } else 1f
    ...
    fun injectGpuInfo(chip: KnownChip, info: GpuInfo) {
        ...
        // ★ Throttling 误判修复 (P1-24): 原代码 frequencyKHz <= 0 时赋值 chip.gpuClockMhz*1000,
        //   与 maxFreqKHz 相等, 导致 DeviceRepository 中
        //   isThrottled = (frequencyKHz < maxFreqKHz * 0.7f) 永远为 false
        //   修复: 仅设置 min/max 频率边界, 不注入当前频率
        //   当前频率应由 sysfs 实时读取, 未知时保持 0 (UI 显示 "--")
