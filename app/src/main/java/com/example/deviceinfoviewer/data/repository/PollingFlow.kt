...
object PollingFlow {
    ...
    fun launchModulePolling(
        ...
        immediate: Boolean = false,  // ★ 默认 false — 避免 immediate=true 双重采集
        fetcher: suspend () -> Unit,
    ): Job = scope.launch(context) {
        intervalFlow.flatMapLatest { delayMs: Long ->
            flow<Unit> {
                // ★ 首次采集融入 while 循环: 第一个 tick 立即执行
                //   从而消除 immediate=true 时 "先 fetcher() 再 while{fetcher()}" 的双重采集
                // ★ 间隔修复 (P1-13): 原首次 remaining=delayMs (不减 elapsed),
                //   导致首次到二次间隔 = delayMs + elapsed1, 比预期长一个采集耗时
                //   修复: 统一用 (delayMs - elapsed), 首次与后续一致
                while (isActive) {
