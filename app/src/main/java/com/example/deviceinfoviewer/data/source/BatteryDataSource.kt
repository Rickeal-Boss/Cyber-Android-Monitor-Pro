...
class BatteryDataSource(private val context: Context) {
    ...
    private fun getBatteryCycleCountFull(): Pair<Int, String> {
        // === Level 0: BatteryManager.getIntProperty(6) — 非标准属性, 多 OEM 返回假值 ===
        // 标准 BatteryManager 属性仅 1-5 (CHARGE_COUNTER/CURRENT_NOW/CURRENT_AVG/CAPACITY/ENERGY_COUNTER)
        // 属性 6 在某些 ROM 上返回温度/阻抗等非循环值 (如固定 5)
        // → 仅当返回值 >= 15 时才采信 (15 次循环以下的设备几乎不存在)
        try {
            val bm = appContext.getSystemService(Context.BATTERY_SERVICE) as? BatteryManager
            if (bm != null) {
                val cycle = bm.getIntProperty(6)
                if (cycle >= 15 && cycle < 10000) {
                    Log.d(TAG, "cycle_count via BatteryManager.getIntProperty(6): $cycle")
                    return Pair(cycle, "BatteryManager (intProperty 6)")
                } else if (cycle in 1..14) {
                    Log.d(TAG, "cycle_count IGNORED: getIntProperty(6)=$cycle (低于阈值15, 疑为假值)")
                }
            }
        } catch (_: Throwable) {}

        // === Level 0.1: BatteryManager 反射 (备用常量名) ===
        try {
            val bm = appContext.getSystemService(Context.BATTERY_SERVICE) as? BatteryManager
            if (bm != null) {
                try {
                    ...
                    if (cycle > 0 && cycle < 10000) {
                        ...
                        return Pair(cycle, "BatteryManager hidden API")
                    }
                } catch (_: Throwable) {}
            }
        } catch (_: Throwable) {}
