...
class BatteryDataSource(private val context: Context) {
    ...
    fun getBatteryInfo(): BatteryInfo {
        ...
        if (cycleCount > 0) {
            Log.w(TAG, "⬅️ cycle_count=$cycleCount from $cycleSource")
        }
    ...
    private fun getBatteryCycleCountFull(): Pair<Int, String> {
        // === Level 0: BatteryManager.getIntProperty(6) — BATTERY_PROPERTY_CYCLE_COUNT === 6, 公开于 API 35 (Android 15) ===
        try {
            val bm = appContext.getSystemService(Context.BATTERY_SERVICE) as? BatteryManager
            if (bm != null) {
                val cycle = bm.getIntProperty(6)  // 6 = BATTERY_PROPERTY_CYCLE_COUNT
