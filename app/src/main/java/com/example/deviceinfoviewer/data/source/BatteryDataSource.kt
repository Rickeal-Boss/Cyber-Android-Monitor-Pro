...
class BatteryDataSource(private val context: Context) {
    ...
    private var lastSystemSOH: Int = -1  // BatteryManager STATE_OF_HEALTH (API 34+)
    ...
    fun getBatteryInfo(): BatteryInfo {
        ...
        // === 系统 SOH (API 34+ BatteryManager STATE_OF_HEALTH) ===
        info.stateOfHealthPercent = lastSystemSOH
    ...
    private fun getBatteryCycleCountFull(): Pair<Int, String> {
        // === Level 0: EXTRA_CYCLE_COUNT from ACTION_BATTERY_CHANGED ===
        // 注: BATTERY_PROPERTY_CYCLE_COUNT 在 AOSP 中不存在 (验证于 2025-03 main 分支)
        // 循环计数仅以 Intent Extra 形式存在，且依赖 OEM Health HAL 实现
        try {
            val intent = appContext.registerReceiver(null, android.content.IntentFilter(android.content.Intent.ACTION_BATTERY_CHANGED))
            val cycle = intent?.getIntExtra("android.os.extra.CYCLE_COUNT", -1) ?: -1
            if (cycle > 0 && cycle < 10000) {
                Log.d(TAG, "cycle_count via ACTION_BATTERY_CHANGED EXTRA_CYCLE_COUNT: $cycle")
                return Pair(cycle, "ACTION_BATTERY_CHANGED")
            }
        } catch (_: Throwable) {}

        // === Level 0b: BATTERY_PROPERTY_STATE_OF_HEALTH (值=10, API 34+) — 系统健康度 ===
        try {
            val bm = appContext.getSystemService(Context.BATTERY_SERVICE) as? BatteryManager
            if (bm != null && android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
                val soh = bm.getIntProperty(android.os.BatteryManager.BATTERY_PROPERTY_STATE_OF_HEALTH)
                if (soh in 1..100) {
                    Log.d(TAG, "BatteryManager STATE_OF_HEALTH: $soh%")
                    lastSystemSOH = soh
                }
            }
        } catch (_: Throwable) {}
