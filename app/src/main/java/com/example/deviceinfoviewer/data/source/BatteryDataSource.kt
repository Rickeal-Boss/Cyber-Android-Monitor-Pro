...
class BatteryDataSource(private val context: Context) {
    ...
    // ★ 性能优化 (2026-06-23): 电池广播缓存
    //   registerReceiver(null, filter) 每次 tick 触发 50-80ms IPC 开销。
    //   系统本身每 2-10s 发送 ACTION_BATTERY_CHANGED 广播，通过持续监听
    //   缓存最新 intent，getBatteryInfo() 直接读缓存 ≈ 0ms。
    @Volatile
    ...
    fun getBatteryInfo(): BatteryInfo {
        ...
        // ★ 优先读缓存，缓存为空时 fallback 到 registerReceiver(null)
        ensureBatteryReceiver()
        val batteryStatus = cachedBatteryIntent
            ?: run {
     ...
     */
    // [Architect Note] current_now 单位判定策略
    //
    // 物理背景:
    //   - 标准 Linux power_supply class 的 current_now 始终为 µA (微安)
    //     Source: AOSP kernel/Documentation/ABI/testing/sysfs-class-power-supply
    //     "current_now: Measured in µA. Negative means discharging from the battery."
    //   - OPPO/OnePlus/Realme 的 oplus_chg 驱动使用 mA (毫安)
    //     Source: OPPO 内核源码 (ColorOS 12/13/14), XDA 多设备实测
    //   - 其他 OEM 专属路径 (vivo_current, batt_current_now 等) 沿用标准 µA
    //
    // 阈值设计:
    //   µA 典型范围: 0 ~ 20,000,000 (0A ~ 20A)
    //   mA 典型范围: 0 ~ 20,000     (0A ~ 20A)
    //   手机电池最大充电电流 ≤ 15A (极端快充), 正常 ≤ 6A
    //   → OPPO_UA_THRESHOLD = 15000: |raw| > 15000 且为 oplus 路径时,
    //     必为 µA (因为 15,000 mA = 15A 已达手机物理上限, 再大不可能是 mA)
    //   → NON_OPPO_MA_THRESHOLD = 50: |raw| < 50 且非 oplus 路径时,
    //     可能是某种 mA 格式 (因 50µA = 0.05mA 在正常采集中几乎不可见)
    //
    // 单元测试验证:
    //   - oplus 路径 raw=1234   → 识别为 mA → 1,234,000 µA (=1.234A) ✓
    //   - oplus 路径 raw=1234567 → 识别为 µA → 1,234,567 µA (=1.235A) ✓
    //   - 标准路径 raw=500000   → 识别为 µA → 500,000 µA (=0.5A)    ✓
    //   - 标准路径 raw=30       → 识别为 mA → 30,000 µA (=0.03A)    ✓
    private companion object CurrentPathRegistry {
        // [Architect Note] 每个路径的已知单位，避免纯启发式误判
        // UnitHint.ASSUME_UA: 标准 Linux power_supply 路径，直接当 µA
        // UnitHint.ASSUME_MA: OEM 驱动已知用 mA 的路径
        // UnitHint.AUTO: 无法确定，用启发式阈值判断
        private enum class UnitHint { ASSUME_UA, ASSUME_MA, AUTO }

        // [Architect Note] 仅在 Companion 内初始化一次，避免每 tick 重建 list
        ...
        /**
         * [Architect Note] 纯函数 — 电流单位转换逻辑，不依赖 Android Context，
         * 便于单元测试验证。输入 rawValue (路径原始值) + pathHint (路径关键词)，
         * 返回转换后的 µA 值。
         *
         * 单元测试验证:
         *   - oplus 路径 raw=1234   → 识别为 mA → 1,234,000 µA
         *   - oplus 路径 raw=1234567 → 识别为 µA → 1,234,567 µA
         *   - 标准路径 raw=500000   → 识别为 µA → 500,000 µA
         *   - 标准路径 raw=30       → 识别为 mA → 30,000 µA
         */
        @kotlin.annotation.VisibleForTesting
    ...
    private fun getCurrentNowFull(): Pair<Long, String> {
        ...
        for ((path, desc) in currentPaths) {
            try {
                ...
                // [Architect Note] 委托纯函数 convertCurrentToMicroamps 做单位转换
