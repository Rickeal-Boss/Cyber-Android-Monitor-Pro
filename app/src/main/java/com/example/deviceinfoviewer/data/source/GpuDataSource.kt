...
class GpuDataSource {
    ...
    fun getGpuInfo(): GpuInfo {
        ...
        // 6. GPU 核心电压 (学术依据: GPULogger 论文, WiMob 2015)
        resolveGpuVoltage(info)
    ...
    // ===== GPU 核心电压 (学术依据: GPULogger 论文验证, WiMob 2015) =====
    private fun resolveGpuVoltage(info: GpuInfo) {
        // 高通 Adreno kgsl
        val kgslPaths = arrayOf(
            "/sys/class/kgsl/kgsl-3d0/gpu_voltage",
            "/sys/class/kgsl/kgsl-3d0/regulator",
            "/sys/class/kgsl/kgsl-3d0/gpu_voltage_mv",
            "/sys/class/kgsl/kgsl-3d1/gpu_voltage"
        )
        for (path in kgslPaths) {
            val v = SysFsReader.readLong(path)
            if (v > 0) { info.gpuVoltageMV = v.coerceIn(100, 2000); return }
        }
        ...
        for (dir in devfreqDirs) {
            if (!dir.lowercase().let { it.contains("gpu") || it.contains("mali") }) continue
            val v = SysFsReader.readLong("/sys/class/devfreq/$dir/cur_voltage")
            if (v > 0) { info.gpuVoltageMV = v.coerceIn(100, 2000); return }
        }

        // Mali 直接路径
        val maliVolts = arrayOf(
            "/sys/kernel/gpu/gpu_voltage",
            "/sys/kernel/gpu/voltage",
            "/proc/mali/voltage"
        )
        for (path in maliVolts) {
            val v = SysFsReader.readLong(path)
            if (v > 0) { info.gpuVoltageMV = v.coerceIn(100, 2000); return }
        }

        // MTK 天玑
        val v = SysFsReader.readLong("/sys/module/ged/parameters/gpu_voltage")
        if (v > 0) { info.gpuVoltageMV = v.coerceIn(100, 2000); return }

        // 通用 kernel gpu 路径
        val raw = SysFsReader.readLine("/sys/kernel/gpu/gpu_voltage")
        if (raw.isNotEmpty()) {
            val match = Regex("""(\d+)""").find(raw.trim())
            match?.groupValues?.get(1)?.toLongOrNull()?.let { mv ->
                if (mv in 100..2000) { info.gpuVoltageMV = mv; return }
            }
        }
    }
}
