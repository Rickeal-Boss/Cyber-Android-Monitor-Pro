...
class GpuDataSource {
    ...
    private fun tryRead(path: String): Long {
        try {
            ...
            val raw = SysFsReader.readLine(path).trim()
            if (raw.isEmpty()) return -1
            // 保留第一个数值（支持 "587000000 Hz", "587 MHz", "587.5" 等格式）
            val numberMatch = Regex("""(\d+\.?\d*)""").find(raw)
            val numStr = numberMatch?.groupValues?.get(1) ?: return -1
            return numStr.toDoubleOrNull()?.toLong() ?: -1
