...
class GpuDataSource {
    ...
    private fun resolveVulkanInfo(info: GpuInfo) {
        ...
        try {
            // ── 方式1: 系统属性 Vulkan 版本 ──
            val apiVer = SysFsReader.readProp("ro.vulkan.version")
            ...
            // ── 方式2: 系统属性 GPU 驱动版本 ──
            val driverVer = SysFsReader.readProp("ro.gpu.driver.version")
            ...
            if (vulkanApiVer.isEmpty()) {
                val glEsVer = SysFsReader.readProp("ro.opengles.version")
                if (glEsVer.isNotEmpty()) {
                    // GLES 3.2 → Vulkan 1.1, GLES 3.1 → Vulkan 1.0
                    val glesNum = glEsVer.trim().toIntOrNull() ?: 0
                    if (glesNum >= 196609) { // 0x30001 = GLES 3.1
                        vulkanApiVer = when { glesNum >= 196610 -> "1.1.0"; else -> "1.0.0" }
                        info.vulkanSource = "OpenGL ES 推断"
                    }
                }
            }

            // ── 方式4: 查找 Vulkan .so 库并以库版本推断 ──
            if (vulkanDriverVer.isEmpty()) {
                try {
                    ...
                    for (libPath in libPaths) {
                        if (SysFsReader.fileExists(libPath)) {
                            info.vulkanSource = "Vulkan 库检测"
                            break
                        }
                    }
                } catch (_: Throwable) {}
            ...
            // ── 方式5: dumpsys gpu 中的 Vulkan 信息 (Android 16+) ──
            if (vulkanApiVer.isEmpty() || vulkanDriverVer.isEmpty()) {
                try {
                    ...
                    for (line in gpuOutput.split("\n")) {
                        if (line.lowercase().contains("vulkan")) {
                            // 格式: "Vulkan: 1.3.0" or "Vulkan API Version: 1.3"
                        ...
                        // 格式: "Driver: 2.0.149" or "driver: 0.803.0"
                        if (line.lowercase().contains("driver") && line.lowercase().contains("version")) {
