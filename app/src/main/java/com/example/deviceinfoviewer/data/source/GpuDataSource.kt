...
class GpuDataSource {
    ...
    // ★ 性能优化 (2026-06-23): 成功路径快速通道
    //   首次 tick 后缓存最后一次成功的频率解析路径，后续直接命中，跳过 8 级 fallback
    //   同时缓存 model/governor（运行时不变），消除冗余 devfreq listDir
    @Volatile private var freqPathCached = false
    private var rapidFreqPath = ""
    private var staticModelReady = false
    private var cachedModel = ""
    private var cachedGovernor = ""
    private var cachedAvailableGovernors = ""
    // ★ 缓存 devfreq 列表，同一 tick 内复用（频率 + governor 共用）
    private var devfreqDirsCache: List<String>? = null

    fun getGpuInfo(): GpuInfo {
        val info = GpuInfo()
        info.timestamp = System.currentTimeMillis()

        // 1. 型号 & 厂商 & OpenGL 渲染器 & 调速器 (静态 — 一次性)
        if (!staticModelReady) {
            resolveGpuModel(info)
            cachedModel = info.model
            resolveGovernor(info)  // ★ 首次同时填充 governor (复用 devfreq 列表)
            cachedGovernor = info.governor
            cachedAvailableGovernors = info.availableGovernors
            staticModelReady = true
        } else {
            info.model = cachedModel
            info.governor = cachedGovernor
            info.availableGovernors = cachedAvailableGovernors
        }
        ...
        // 5. Vulkan 驱动版本 (内部已缓存 voltaile 标志)
        resolveVulkanInfo(info)

        // ★ 清理 tick 级缓存
        devfreqDirsCache = null
    ...
    // ===== GPU 频率 (动态优先 + 快速通道 + 负载估算兜底) =====
    private fun resolveGpuFrequency(info: GpuInfo) {
        // ★ Phase 0: 快速通道 — 上次成功的路径直接命中 (跳过全扫描)
        if (freqPathCached && tryRapidPath(info)) return

        // ★ 全量扫描时需要 devfreq 列表 (频率 + governor 共用)
        val devfreqDirs = cachedDevfreqList()

        // Phase 1: 最大频率
        resolveMaxFrequency(info, devfreqDirs)

        // Phase 2: 全路径扫描 + 缓存成功路径
        // ═══════════════ 高通 Adreno ═══════════════
        if (tryQualcommKgsl(info)) { saveFreqPath("kgsl"); return }
        if (tryQualcommExpanded(info)) { saveFreqPath("kgsl_expanded"); return }

        // ═══════════════ ARM Mali devfreq ═══════════════
        if (tryMaliDevfreq(info, devfreqDirs)) { saveFreqPath("mali_devfreq"); return }

        // ═══════════════ Mali 扩展路径 ═══════════════
        if (tryMaliDirect(info)) { saveFreqPath("mali_direct"); return }

        // ═══════════════ MTK 天玑 ═══════════════
        if (tryMtk(info, devfreqDirs)) { saveFreqPath("mtk"); return }

        // ═══════════════ 通用 /sys/kernel/gpu/ ═══════════════
        if (tryKernelGpu(info)) { saveFreqPath("kernel_gpu"); return }

        // ═══════ 以下重型 fallback 仅首次使用 — 成功后缓存路径 ═══════
        if (tryShellSysfsGpu(info)) { saveFreqPath("shell_sysfs"); return }
        if (tryDumpsysGpu(info)) { saveFreqPath("dumpsys"); return }
        if (estimateFromLoad(info)) { saveFreqPath("estimate"); return }
        if (tryDynamicProperty(info)) { saveFreqPath("prop"); return }

        tryPropertyFallback(info)
    }

    /** ★ 快速通道: 跳过 fallback 链，直接用缓存路径 */
    private fun tryRapidPath(info: GpuInfo): Boolean {
        return when (rapidFreqPath) {
            "kgsl" -> tryQualcommKgsl(info)
            "kgsl_expanded" -> tryQualcommExpanded(info)
            "mali_devfreq" -> tryMaliDevfreq(info, cachedDevfreqList())
            "mali_direct" -> tryMaliDirect(info)
            "mtk" -> tryMtk(info, cachedDevfreqList())
            "kernel_gpu" -> tryKernelGpu(info)
            "shell_sysfs" -> tryShellSysfsGpu(info)
            "dumpsys" -> tryDumpsysGpu(info)
            "estimate" -> estimateFromLoad(info)
            "prop" -> tryDynamicProperty(info)
            else -> false
        }
    }

    /** ★ 获取缓存 devfreq 列表 (同一 tick 内复用) */
    private fun cachedDevfreqList(): List<String> {
        return devfreqDirsCache ?: SysFsReader.listDir("/sys/class/devfreq/").also { devfreqDirsCache = it }
    }

    private fun saveFreqPath(path: String) {
        rapidFreqPath = path
        freqPathCached = true
    }
    ...
    private fun resolveMaxFrequency(info: GpuInfo, devfreqDirs: List<String>) {
        // [Architect Note] GPU 最大频率路径 (来源标注)
        // Source: CodeLinaro kernel/msm-5.x/drivers/gpu/msm/kgsl.c
        //   /sys/class/kgsl/kgsl-3d0/max_gpuclk — Qualcomm Adreno GPU 最大频率 (Hz)
        //   /sys/class/kgsl/kgsl-3d0/devfreq/max_freq — Qualcomm devfreq 最大频率
        // Source: Android Googler kernel — /sys/class/devfreq/ 通用 DVFS 接口
    ...
    private fun tryMaliDevfreq(info: GpuInfo, devfreqDirs: List<String>): Boolean {
        for (dir in devfreqDirs) {
    ...
    private fun tryMtk(info: GpuInfo, devfreqDirs: List<String>): Boolean {
    ...
    private fun resolveGovernor(info: GpuInfo) {
        ...
        if (gov.isNotEmpty()) { info.governor = gov; cacheGovernor(info) }
    }

    private fun cacheGovernor(info: GpuInfo) {
        cachedGovernor = info.governor
        cachedAvailableGovernors = info.availableGovernors
    }
    ...
    private fun resolveLoad(info: GpuInfo) {
        ...
        if (gpuBusy.isNotEmpty()) {
            val parts = gpuBusy.trim().split(WS)
            if (parts.size >= 2) {
                try {
        ...
        for (dir in dirs) {
            ...
            if (parts.size == 1) parts = loadStr.split(WS)
            for (part in parts) {
    ...
    companion object {
        ...
        private val NOT_DIGIT = Regex("[^0-9]")  // ★ 预编译
    ...
    private fun formatVulkanVersion(raw: String): String {
        ...
        val parts = cleaned.split(WS).firstOrNull()?.split(".") ?: return raw.trim()
