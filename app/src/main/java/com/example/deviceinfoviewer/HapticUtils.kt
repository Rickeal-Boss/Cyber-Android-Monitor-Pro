...
object HapticUtils {
    ...
    fun stepTick(context: Context) {
        ...
        val effect = when (cachedIntensity) {
            1 -> VibrationEffect.createOneShot(8, VibrationEffect.DEFAULT_AMPLITUDE / 2)
            2 -> VibrationEffect.createOneShot(12, VibrationEffect.DEFAULT_AMPLITUDE / 2)
            else -> VibrationEffect.createOneShot(16, VibrationEffect.DEFAULT_AMPLITUDE / 2)
        }
    ...
    private fun vibrate(context: Context, type: Int) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
            ...
            @Suppress("DEPRECATION") vibe.vibrate(dur)
            return
        }
