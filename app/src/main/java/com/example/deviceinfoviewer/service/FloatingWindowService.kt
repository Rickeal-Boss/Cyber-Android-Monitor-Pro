...
class FloatingWindowService : Service() {
    ...
    private val itemDefs = listOf(
        "cpu_usage"    to { makeItem("cpu_usage", "CPU: --%", 16, 200) },
        "gpu_usage"    to { makeItem("gpu_usage", "GPU: --%", 16, 260) },
        "cpu_temp"     to { makeItem("cpu_temp", "CPU: --°C", 16, 320) },
        "gpu_temp"     to { makeItem("gpu_temp", "GPU: --°C", 16, 380) },
        "cpu_freq"     to { makeItem("cpu_freq", "C0: --MHz\nC1: --MHz\nC2: --MHz\nC3: --MHz", 16, 440) },
        "ram"          to { makeItem("ram", "内存: --%", 16, 620) },
        "battery_temp" to { makeItem("battery_temp", "电池: --°C", 16, 680) },
        "battery_cur"  to { makeItem("battery_cur", "电流: --mA", 16, 740) },
        "battery_pow"  to { makeItem("battery_pow", "功率: --W", 16, 800) },
        "fps"          to { makeItem("fps", "FPS: --", 16, 860) }
    )
    ...
    private fun refreshData(
        ...
        val curText = when {
            bat.currentNowUA > 0 -> "充电: ${bat.currentNowUA / 1000}mA"
            bat.currentNowUA < 0 -> "放电: ${-bat.currentNowUA / 1000}mA"
        ...
        if (effV > 0 && curUA != 0L) {
            ...
            setText("battery_pow", "功率: ${"%.2f".format(powerW)}W" +
                if (isChargingBat) " ↑" else " ↓")
        } else {
            setText("battery_pow", "功率: --W")
        }
    }
