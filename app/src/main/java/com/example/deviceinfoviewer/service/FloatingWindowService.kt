...
/**
 * 悬浮窗前台服务 v3 — 8 种实时指标 + FPS + 每指标独立刷新间隔 + RefreshPolicy 统一调速
 * 
 * 前后台自动同步 (2026-06-21):
 * - 后台时 baseTickMs 强制 ≥ RefreshPolicy.BACKGROUND_CAP_MS (5000ms)
 * - 前台时恢复正常的 min(visible intervals) 动态计算
 * - 无需外部手动控制，内部读取 RefreshPolicy.isBackground
...
class FloatingWindowService : Service() {
    ...
    override fun onCreate() {
        ...
        try {
            ...
            // ★ 预缓存所有静态标签 (避免每 200ms tick 调用 getString)
    ...
    private fun startUpdating() {
        refreshRunnable = object : Runnable {
            override fun run() {
                ...
                dataExecutor.execute {  // ★ 复用单线程池，避免每 tick new Thread()
                    try {
                ...
                }  // dataExecutor.execute block end
     ...
     *  ★ 后台时强制不低于 RefreshPolicy.BACKGROUND_CAP_MS (5000ms)，实现统一节能 */
    private fun computeBaseTickMs(): Long {
        ...
        val computedMin = intervals.min()
        // ★ 前台: 下限 200ms，后台: 强制不低于 5000ms (与 DeviceRepository 同步)
        val floorMs = if (RefreshPolicy.isBackground) RefreshPolicy.BACKGROUND_CAP_MS else 200L
        return computedMin.coerceIn(floorMs, 30000L)
    }
