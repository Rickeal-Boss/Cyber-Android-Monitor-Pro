...
/**
 * 悬浮窗前台服务 v6 — DeviceRepository 订阅模式
 *
 * [Architect Note] P0-5 修复 (2026-06-23):
 * - 消除独立 DataSource 实例和 handler 轮询循环
 * - 改为订阅 DeviceRepository 的 SharedFlow (combine 4 路数据)
 * - DeviceRepository 在 Dispatchers.IO 统一采集 + CpuCache 注入
 * - FloatingWindowService 仅消费数据 → 更新 UI TextViews
 *
 * 历史架构:
 * - v5: 串行采集 + CpuCache 机型优化 + tickInFlight 防堆积
 * - v4: 去每指标独立间隔，统一 RefreshPolicy 驱动
 * - v3: 单线程池复用 + 静态标签预缓存
 */
class FloatingWindowService : Service() {
    ...
    // [Architect Note] P0-5 修复: 改为订阅 DeviceRepository 的 SharedFlow，
    // 消除独立 DataSource 导致的 sysfs 双重读取。
    // FloatingWindowService 仅负责 UI 呈现，数据采集统一由 DeviceRepository 管理。
    private lateinit var repo: DeviceRepository

    // FPS (Choreographer 独立驱动，不依赖 DeviceRepository)
    ...
    override fun onCreate() {
        ...
        try {
            ...
            // [Architect Note] P0-5 修复: 通过 Application 获取 DeviceRepository 单例，
            // 订阅其 SharedFlow 获取预采集数据，消除 sysfs 双重读取
            ...
            serviceScope.launch {
                combine(
                    ...
                    if (newTick != baseTickMs) {
                        baseTickMs = newTick
                        // [Architect Note] P0-5: 间隔变更时重启 Flow 订阅
                        stopDataCollection()
                        startDataCollection()
                    }
                }
    ...
    // ═══════ Flow 订阅采集 — 替代 startUpdating 轮询循环 ═══════
    // [Architect Note] P0-5 修复: 订阅 DeviceRepository 的 SharedFlow，
    // 消除独立 DataSource 实例和 handler 轮询。DeviceRepository 已经
    // 在正确的协程上下文 (Dispatchers.IO) 中执行采集 + CpuCache 注入，
    // FloatingWindowService 仅消费最终数据 → 更新 UI TextViews。
    private fun startDataCollection() {
        collectionJob = serviceScope.launch {
            // combine 4 个 SharedFlow，任一更新时重新渲染
            combine(
                repo.cpuFlow,
                repo.gpuFlow,
                repo.batteryFlow,
